...
function initHttpServer(app) {
    app.use(function (req, res, next) {
        var ruleValue = getRuleValue(req),
            ...
            fullUrlArr = fullUrl.split('??'),
            preString = fullUrlArr[0].split('i.alicdn.com/')[1],
            jsName = (fullUrlArr[1]).split('.')[0],
            //vueJsSourceFile = 'http://127.0.0.1:' + ruleValue + '/.build/' + preString + jsName + '.js'
            vueJsSourceFile = 'http://127.0.0.1:' + ruleValue + '/' +jsName + '/' + jsName + '.js'
